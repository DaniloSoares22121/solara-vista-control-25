...
-- Primeiro, verifica e remove tabelas existentes se houver problemas
DROP TABLE IF EXISTS public.rateio_items CASCADE;
DROP TABLE IF EXISTS public.rateios CASCADE;

-- Cria a tabela rateios com a estrutura correta
...
-- Cria a tabela rateio_items com foreign key correta
...
CREATE INDEX idx_rateios_user_id ON public.rateios(user_id);
CREATE INDEX idx_rateios_geradora_id ON public.rateios(geradora_id);
CREATE INDEX idx_rateio_items_rateio_id ON public.rateio_items(rateio_id);
CREATE INDEX idx_rateio_items_assinante_id ON public.rateio_items(assinante_id);
